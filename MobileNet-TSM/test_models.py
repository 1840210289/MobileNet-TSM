...
# parser.add_argument('--weights', type=str, default='pretrained/TSM_kinetics_RGB_resnet50_shift8_blockres_avg_segment8_e50.pth')
parser.add_argument('--weights', type=str, default="./TSM-3/4/HockeyFights/ckpt.best.pth.tar")
...
# for true test
parser.add_argument('--test_list', type=str, default='G:/RWF-2000/val_videofolder.txt')
...
# parser.add_argument('--dataset', default='RWF-2000')
...
# weights_list = args.weights.split('.')
# test_segments_list = [int(s) for s in args.test_segments]
test_segments_list = [int(s) for s in args.test_segments.split(',')]
print(weights_list)
print(len(weights_list))
print(test_segments_list)
print(len(test_segments_list))
assert len(weights_list) == len(test_segments_list)
# assert len(weights_list) == test_segments_list
...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    ...
    # base_dict = {'.'.join(k.split('.')[1:]): v for k, v in list(checkpoint.items()) if (k in model_dict and 'new_fc' not in k)}
    base_dict = {'.'.join(k.split('.')[1:]): v for k, v in list(checkpoint.items()) if not ('new_fc' in k)}
